...
function addSidebarToggleListener() {
    ...
    // *************************** scan for ticket numbers ***************************

    let ticketNumbers = new Set(); // Use a Set to store unique ticket numbers
    let scanInterval = 5000; // Check every 5 seconds
    let scanTimer;
    ...
    function getTicketNumbers() {
        ...
        $('#ticket tbody tr').each(function() {
            let td = $(this).find('td:eq(1)'); // Select the second TD
            ...
            if (td.length) {
                ...
                if (span.length) {
                    ticketNumber = span.attr('title') || span.text().trim();
                } else if (link.length) {
                    let match = link.attr('href').match(/ID=(\d+)/);
                    if (match) ticketNumber = match[1];
                }
            }

            if (ticketNumber && !ticketNumbers.has(ticketNumber)) {
        ...
        if (newNumbersFound) {
            console.log("Current Ticket Numbers:", Array.from(ticketNumbers));
        }

        return newNumbersFound;
    }

    function startScanning() {
        // Periodically scan the ticket table every `scanInterval` milliseconds
        scanTimer = setInterval(function() {
            let newNumbersFound = getTicketNumbers();
            if (!newNumbersFound) {
                console.log("No new tickets found, continuing periodic scan.");
            }
        }, scanInterval);
    }

    // Start scanning when the page is loaded and continues periodically
